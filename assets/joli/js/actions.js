$(document).ready(function () {
	...
	$("html").on("click", function () {
		if (html_click_avail)
			$(".x-navigation-horizontal li,.x-navigation-minimized li").removeClass(
				"active"
			);
	});
	...
	$(".toggle").on("click", function () {
		...
		if (elm.is(":visible")) elm.addClass("hidden").removeClass("show");
		else elm.addClass("show").removeClass("hidden");
...
function page_content_onresize() {
	$(
		".page-content,.content-frame-body,.content-frame-right,.content-frame-left"
	)
		.css("width", "")
		.css("height", "");
	...
	if ($(window).width() > 1024) {
		...
		if ($(".content-frame-body").height() < $(document).height() - 162) {
			$(".content-frame-body,.content-frame-right,.content-frame-left").height(
				$(document).height() - 162
			);
		} else {
			$(".content-frame-right,.content-frame-left").height(
				$(".content-frame-body").height()
			);
		}
...
function panel_fullscreen(panel) {
	if (panel.hasClass("panel-fullscreened")) {
		...
		panel
			.addClass("panel-fullscreened")
			.wrap('<div class="panel-fullscreen-wrap"></div>');
		panel
			.find(".panel-fullscreen .fa")
...
function panel_collapse(panel, action, callback) {
	if (panel.hasClass("panel-toggled")) {
		...
		panel
			.find(".panel-collapse .fa-angle-up")
		...
		panel
			.find(".panel-collapse .fa-angle-down")
...
function panel_refresh(panel, action, callback) {
	if (!panel.hasClass("panel-refreshing")) {
		panel.append(
			'<div class="panel-refresh-layer"><img src="img/loaders/default.gif"/></div>'
		);
		panel
			.find(".panel-refresh-layer")
			.width(panel.width())
			.height(panel.height());
...
function x_navigation_onresize() {
	...
	if (inner_port < 1025) {
		...
		$(".x-navigation-horizontal").each(function () {
			if (!$(this).hasClass("x-navigation-panel")) {
				$(".x-navigation-horizontal")
					.addClass("x-navigation-h-holder")
					.removeClass("x-navigation-horizontal");
			}
		});
	} else {
		...
		$(".x-navigation-h-holder")
			.addClass("x-navigation-horizontal")
...
function x_navigation() {
	...
	$(".x-navigation li").click(function (event) {
		...
		if (
			...
			if ($(this).hasClass("xn-profile") > 0) return true;
			else return false;
		}
	});
